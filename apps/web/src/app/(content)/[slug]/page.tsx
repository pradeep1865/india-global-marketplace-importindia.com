...
const pageContent: Record<string, { intro: string; sections: Array<{ title: string; body: string }> }> = {
  "about-us": {
    intro: "Emitrix connects global buyers with verified Indian manufacturers, electronics suppliers, exporters, and trade-ready businesses.",
    sections: [
  ...
  "company-policy": {
    ...
    sections: [
      ...
      { title: "Marketplace enforcement", body: "Emitrix may hold listings, reject manufacturers, restrict accounts, or escalate suspicious payment, fraud, counterfeit, or misrepresentation signals to the admin team." }
    ]
  },
  "return-policy": {
  ...
  "contact-us": {
    intro: "Contact Emitrix for sourcing support, manufacturer verification, order concerns, payment questions, listing review, or partnership inquiries.",
    sections: [
  ...
  "privacy-policy": {
    intro: "Emitrix collects account, location, marketplace, payment, and communication data needed to operate a secure sourcing platform.",
    sections: [
    ...
    <>
      ...
      <main className="mx-auto flex max-w-7xl gap-6 px-4 pb-24 pt-6">
        ...
        <article className="w-full rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <p className="font-black uppercase text-[#ff9f1c]">Emitrix</p>
          ...
          <div className="mt-7 grid gap-4">
            {(pageContent[slug]?.sections ?? [{ title: "Marketplace standard", body: "Emitrix keeps this information page available for users, search engines, legal review, and support workflows." }]).map((section) => (
